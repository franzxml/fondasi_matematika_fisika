...
function genType1(){
  $a=randInt(-9,9); while($a===0)$a=randInt(-9,9);
  $x=randInt(-9,9);
  $b=$x+$a;
  $eq="x ".sgn($a)." = ".$b;
  $steps=["Kurangi ".abs($a)." di kedua sisi: x = ".$b." ".($a>=0?"- ":"+ ").abs($a),"Hitung: x = ".($b-$a)];
  return [$eq,$x,$steps,"x + a = b"];
}

function genType2(){
  $k=pick([2,3,4,5,-2,-3]); $x=randInt(-9,9); $b=$k*$x;
  $eq=$k."x = ".$b;
  $steps=["Bagi kedua sisi dengan ".abs($k).": x = ".$b." / ".$k,"Hitung: x = ".($b/$k)];
  return [$eq,$x,$steps,"kx = b"];
}

function genType3(){
  $k=pick([2,3,4,-2,-3]); $x=randInt(-8,8); $a=randInt(-9,9); $b=$k*$x+$a;
  $eq=$k."x ".sgn($a)." = ".$b;
  $steps=["Pindahkan konstanta: ".$k."x = ".$b." ".($a>=0?"- ":"+ ").abs($a),"Sederhanakan: ".$k."x = ".($b-$a),"Bagi ".abs($k).": x = ".($b-$a)." / ".$k,"Hitung: x = ".(($b-$a)/$k)];
  return [$eq,$x,$steps,"kx + a = b"];
}

function genType4(){
  $a=pick([2,3,4,5,-2,-3]); $c=pick([0,1,2,3,-1,-2]); if($c===$a)$c=$a+1;
  $x=randInt(-6,6); $b=randInt(-9,9); $d=$a*$x+$b-$c*$x;
  $eq=$a."x ".sgn($b)." = ".$c."x ".sgn($d);
  $steps=["Kumpulkan x: ".($a-$c)."x ".sgn($b)." = ".$d,"Pindahkan konstanta: ".($a-$c)."x = ".$d." ".($b>=0?"- ":"+ ").abs($b),"Sederhanakan: ".($a-$c)."x = ".($d-$b),"Bagi ".abs($a-$c).": x = ".($d-$b)." / ".($a-$c),"Hitung: x = ".(($d-$b)/($a-$c))];
  return [$eq,$x,$steps,"ax + b = cx + d"];
}

function genType5(){
  $p=pick([2,3,-2]); $q=pick([1,2,3]); $r=randInt(-5,5); $s=randInt(-6,6); $t=pick([0,1,2,3,-1,-2]); $x=randInt(-5,5);
  $u=$p*($q*$x+$r)+$s-$t*$x;
  $eq=$p."(".$q."x ".sgn($r).") ".sgn($s)." = ".$t."x ".sgn($u);
  $A=$p*$q; $B=$p*$r+$s;
  $steps=["Distribusi: ".$A."x ".sgn($p*$r)." ".sgn($s)." = ".$t."x ".sgn($u),"Gabungkan: ".$A."x ".sgn($B)." = ".$t."x ".sgn($u),"Kumpulkan x: ".($A-$t)."x ".sgn($B)." = ".$u,"Pindahkan konstanta: ".($A-$t)."x = ".$u." ".($B>=0?"- ":"+ ").abs($B),"Sederhanakan: ".($A-$t)."x = ".($u-$B),"Bagi ".abs($A-$t).": x = ".($u-$B)." / ".($A-$t),"Hitung: x = ".(($u-$B)/($A-$t))];
  return [$eq,$x,$steps,"p(qx + r) + s = tx + u"];
}

function genType6(){
  $m=pick([2,3,4,5]); $a=randInt(-6,6); $x=randInt(-6,6); $b=$x/$m+$a;
  while(abs($b*10-round($b*10))>1e-9){$x=randInt(-6,6);$b=$x/$m+$a;}
  $b=(string)$b;
  $eq="x/".$m." ".sgn($a)." = ".$b;
  $steps=["Pindahkan konstanta: x/".$m." = ".$b." ".($a>=0?"- ":"+ ").abs($a),"Sederhanakan: x/".$m." = ".($b-$a),"Kalikan ".$m." di kedua sisi: x = ".$m."·".($b-$a),"Hitung: x = ".($m*($b-$a))];
  return [$eq,$x,$steps,"x/m + a = b"];
}
...
<title>Latihan Aljabar Dasar</title>
...
.logo{width:36px;height:36px;border-radius:10px;background:linear-gradient(135deg,#60a5fa,#22d3ee);box-shadow:0 10px 20px rgba(34,211,238,.3)}
...
.select{appearance:none;background:#0b1220;border:1px solid rgba(148,163,184,.25);color:#e2e8f0;border-radius:10px;padding:10px 12px;width:100%}
.problem{font-size:32px;font-weight:700;margin:6px 0 4px}
.meta{opacity:.8;font-size:13px}
.input{width:100%;padding:12px 14px;border-radius:10px;background:#0b1220;border:1px solid rgba(148,163,184,.25);color:#e2e8f0;font-size:18px}
...
<div class="shell">
  ...
  <div class="card">
    ...
    <div class="footer">
      <div>Gunakan urutan: kumpulkan x → pindahkan konstanta → bagi koefisien.</div>
...
function toggleHints(){
  ...
  if(h.classList.contains("show")) window.scrollTo({top:document.body.scrollHeight,behavior:"smooth"});
}
</script>
</body>
</html>